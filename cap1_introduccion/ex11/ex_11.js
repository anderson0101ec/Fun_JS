...
let pais, edad;
pais = prompt('Ingrese su pais de prosedencia...');
...
// Comvertir los valores digitados por el usuario a enteros
day = parseInt (day);
month = parseInt (month);
year = parseInt (year);

// validar que los valores ingresados sean validos
// isNaN es solo para dijitar numeros
if (isNaN(day) || isNaN(month) || isNaN(year)){
    document.writeln('Porfavor solo ingrese valores numericos para el dia, el mes y el año');
} else if (month == 1 || month == 2 || month == 3) {
    document.writeln('Pertenece al primer trimestre del año');
} else {
    document.writeln('No pertenece al primer trimestre de año');
}
